...
describe('UploadsService', () => {
  it('rejects empty file uploads', async () => {
    const service = new UploadsService({} as PrismaService, new ConfigService());

    await expect(service.uploadFile('u1', 'IMAGE', undefined)).rejects.toBeInstanceOf(BadRequestException);
  });

  it('records uploaded COS file metadata', async () => {
    const prisma = {
      attachment: {
        create: jest.fn().mockResolvedValue({ id: 'a1', url: 'https://cdn.example.com/key.jpg' }),
      },
    };
